...
describe("applySkillsPromptLimits (via buildWorkspaceSkillsPrompt)", () => {
  ...
  it("budget check uses the same absolute paths rendered in the prompt", () => {
    // Skill locations are machine instructions for read-tool calls, so keep them
    // absolute rather than compacting to `~` (tool-side home expansion can differ
    // from the managed skill root).
    ...
    const absoluteCompactLen = formatSkillsCompact(skills).length;
    const budget = absoluteCompactLen + 150;
    const prompt = buildPrompt(skills, { maxChars: budget });
    ...
    expect(prompt).not.toContain("included");
    expect(prompt).toContain("compact format");
    // Verify prompt locations remain absolute.
  ...
  it("resolvedSkills and prompt both keep canonical absolute paths", () => {
    const home = os.homedir();
    ...
    // Prompt should use absolute paths so tool calls do not depend on `~` expansion.
